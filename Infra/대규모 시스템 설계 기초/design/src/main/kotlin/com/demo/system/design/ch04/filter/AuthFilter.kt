...
class AuthFilter : AbstractGatewayFilterFactory<AuthFilter.Config>(Config::class.java) {
    ...
    override fun apply(config: Config): GatewayFilter {
        return GatewayFilter { exchange, chain ->
            ...
            return@GatewayFilter this.responseInvalidAutoToken(response)
        }
    }

    private fun hasValidAuthToken(request: ServerHttpRequest, whiteList: Map<String, String>): Boolean {
        val authToken = request.headers.getFirst("X-AUTO-TOKEN")
    ...
    private fun responseInvalidAutoToken(response: ServerHttpResponse): Mono<Void> {
        ...
        return response.writeWith(
            Mono.just(
                response.bufferFactory().wrap(
                    ...
                    |}
                    """.trimIndent().trimMargin().toByteArray()
                )
            )
        )
    }
