...
- 그렇기 때문에 애플리케이션은 기록 위치가 로컬인지 외부인지는 의식할 필요가 없다.
- 로컬 디스크는 소수의 디스크로 RAID를 구성하고 캐시로는 서버의 OS 메모리를 이용한다.
- 외부 저장소는 수십대에서 수백 대 단위의 디스크를 배치하고 캐시 전용 메모리를 사용한다.
- 디스크 수에 따라 처리량이 증가하므로 처리량 관점에서는 외부 저장소가 압도적으로 유리하다.
- 물론 응답속도 자체는 내부 로컬 저장소가 더 빠르다.

**순차 I/O와 랜덤 I/O**
- 디스크 I/O 에는 순차(Sequential) 엑세스와 랜덤(Random) 엑세스가 있다.
- 순차는 선두부터 차례대로 엑세스(읽기/쓰기)하는 방식
- 랜덤 엑세스는 헤드가 움직이면서 해당 위치로 건너뒤는 엑세스(읽기/쓰기) 방식
- 순차 엑세스는 고속으로 한번에 데이터를 읽어 들일수 있고, 랜덤 엑세스는 항상 해당 부분을 찾아서 데이터를 읽어야 한다.
- 단일 디스크가 기록 위치인 경우는 순차 방식이 빠르고 랜덤은 느리다.
- 오라클 DB 에서는 랜덤 특성에 의해 발생하는 속도 저하를 해결하기 위해 데이터 파일에 기록을 담당하는 DBWR 프로세스를 다중화 한다.
- 옛날에 조각모음이란 기능을 했던 이유는 디스크에 퍼져있는 데이터들을 모아서 랜덤 엑세스효율을 높이기 위해서이다.

### 8.3.4 네티워크 I/O 병목 현상
- CPU, 메모리 보다 응답 시간 오버헤드가 크다.
- 근본적 개선이 매우어렵다

**통신 프로세스의 병목 현상**
- 네트워크 회선에서 꼭 대역이 크다고 해서 고속 통신을 할 수있는건 아니다. 
- 하나의 프로세스로 처리하는 경우 높은 처리량을 실현하는 것은 매우 어렵다.
...
- 책에 나와있는 예제는 게이트웨이로 사용중인 라우터가 있고, 라우터의 처리 한계에 다다르는 경우 실제 어디서 병목이 발생하는지 찾기 힘들수도 있다. 
- 그렇기 때문에 시스템 구축시 경로와 트래픽 증감에 대해서는 검토해야한다.
  

### 8.3.5 애플리케이션 병목 현상 예
- 알고리즘 문제라면 앞에 나와있는 인프라적 문제들을 해결해도 처리량이 높아지지 않는다.

**데이터 갱신의 병목 현상**
- 특정 데이터에 의존하는 처리가 병목 지점이 됨
- 특정 데이터에 배타적 제어가 있다면 아무리 많은 병렬 처리를 해도 처리량은 늘어지 않는다.

- **값의 캐시화**
  - 더 가까운  장소에 캐시화를 해서 네트워크 경유를 제거할 수 있다.
  - 다만 병목 지점이란는 것에는 변함이 없어서 근본적인 해결책은 되지 못한다.
- **병목 지점의 분할**
  - 대상 데이터를 두개로 분할 할 수도 있다.
  - 다만 이런경우는 데이터의 일치성 문제와 최신성 문제가 발생할 수 있다.
  - 그렇기 때문에 복제를 고려해야 한다.

**외부 질의의 병목 현상**
- 다른 시스템에 의존하는 부분이 병목 현상이 될 수 있다.
- 예로 인증서버 처리량이 느리다면 인증을 이용하는 모든 처리에서 병목 현상이 발생할 수 있다.
